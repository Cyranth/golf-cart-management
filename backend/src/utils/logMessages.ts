...
const { API_PORT, NODE_VERSION } = process.env;

export const SERVER_START = `${c.LBlue}Express Server started on port ${c.Reset}${API_PORT}${c.LBlue}${c.Reset}.`;
...
export const UTC_DATE_TIME = `${c.Yellow}[UTC]${c.Reset
  } Current date and time: ${getCurrentDateTime().formattedDateUTC}, ${getCurrentDateTime().formattedTimeUTC
  }`;

// Current date time message in PT/PST.
export const PACIFIC_DATE_TIME = `${c.Yellow}[${getCurrentDateTime().pacificTimeZone
  }]${c.Reset} Current date and time: ${getCurrentDateTime().formattedDatePacific
